...
void Encoder_Init_TIM2(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1;					//端口配置PA0(TIM1_CH1) PA1(TIM4_CH2)
...
void Encoder_Init_TIM4(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;					//端口配置 PB6(TIM4_CH1) PB7(TIM4_CH2)
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 				//浮空输入
  ...
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;				//选择时钟分频：不分频
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;		//TIM向上计数
  TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);								//按设定值初始化定时器
  TIM_EncoderInterfaceConfig(TIM4, TIM_EncoderMode_TI12, TIM_ICPolarity_Rising, TIM_ICPolarity_Rising);//使用编码器模式3
  TIM_ICStructInit(&TIM_ICInitStructure);		//按缺省值填入
  TIM_ICInitStructure.TIM_ICFilter = 10;		//选择输入比较滤波器
  TIM_ICInit(TIM4, &TIM_ICInitStructure);		//输入捕获
  TIM_ClearFlag(TIM4, TIM_FLAG_Update);			//清除TIM的更新标志位
  TIM_ITConfig(TIM4, TIM_IT_Update, ENABLE);//使能TIM4更新中断

  TIM_SetCounter(TIM4,0);										//复位计数器
