...
void PWM_init(void)  //霍尔元件初始化  使用PA12io口
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4;			    //PA12 端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	 //IO口速度为50MHz
  GPIO_Init(GPIOA, &GPIO_InitStructure);			     //初始化GPIOA12
  GPIO_SetBits(GPIOB,GPIO_Pin_3);					//PA12输出高
	GPIO_ResetBits(GPIOB,GPIO_Pin_4);        //PA10输出低
...
{  
   ...
   //初始化TIM3
	...
	TIM_OC3Init(TIM2, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM3 OC2            //B5
...
{  
   ...
   //初始化TIM3
	...
	TIM_OC2Init(TIM3, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM3 OC2            //B5
	TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM3在CCR2上的预装载寄存器
	
	TIM_OC3Init(TIM3, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM3 OC3            //B0
	TIM_OC3PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM3在CCR3上的预装载寄存器
	
	TIM_OC4Init(TIM3, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM3 OC4            //B1
